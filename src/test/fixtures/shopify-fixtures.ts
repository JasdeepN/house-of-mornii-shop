// Test-only fixture data for Shopify products & collections.
// Used exclusively by unit tests to mock `shopifyFetch` responses in token mode.
...
// Back-fill each product's `collections` field to mirror the live
// Storefront API's `product.collections` connection in test fixtures.
for (const collection of collections) {
  for (const { node: product } of collection.products.edges) {
    product.collections = {
      edges: [{ node: { handle: collection.handle, title: collection.title } }],
    }
  }
}
...
export function getFixtureCollections(): ShopifyCollection[] {
  return collections
}

export function getFixtureCollection(handle: string): ShopifyCollection | null {
  return collections.find((c) => c.handle === handle) ?? null
}

export function getFixtureProduct(handle: string): ShopifyProduct | null {
